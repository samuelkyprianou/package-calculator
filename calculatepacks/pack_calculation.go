...
import "github.com/samuelkyprianou/package-calculator/utils"
...
func recursive(oQuantity int, packSizes []int, accumulatedSizes []int) []int {
	sum := utils.GetSum(accumulatedSizes) // Calculate the sum of accumulated sizes
	...
	for i, size := range packSizes {
		prevNumsSum := utils.GetSum(packSizes[0 : i+1]) // Calculate the sum of pack sizes up to the current index

		// Check various conditions to decide whether to append the size and recurse
		if sum+size >= oQuantity || (prevNumsSum+sum >= oQuantity) || i == len(packSizes)-1 || (utils.GetSum(packSizes[i+1:i+2])-size >= oQuantity) {
...
func getOptimalSlice(arr [][]int) []int {
	...
	var optimalArrSum int = utils.GetSum(arr[0]) // Initialize optimal sum with the sum of the first slice
	
	for _, array := range arr {
		sum := utils.GetSum(array) // Calculate the sum of the current slice
...
func CalculatePacks(oQuantity int, sizes map[int]int) map[int]int{
	packSizes := utils.OrderPackSizes(sizes) // Order the pack sizes
	reversePackSizes := utils.Reverse(packSizes) // Reverse the ordered pack sizes
